...
class ActorCritic(nn.Module):
    ...
    def __init__(self,  num_proprio,                        # NEW (renamed)       
        ...
        if self.num_scan_obs == 0:
            print("=== SCAN ENCODER DISABLED! (num_scan_obs=0) ===")
        ...
        # [cur. obs | obs. history | latent | scan (optional) | estimated_obs ]
        if self.num_scan_obs == 0:
            mlp_input_dim_a = num_proprio + (num_proprio*history_buffer_length) + latent_encoder_output_dim + num_estimated_obs
        else:
        ...
        # Init adaptation encoder (student)
        ...
        # Init privileged encoder (teacher)
        ...
        # Init scan encoder (vision)
        self.scan_encoder = None if self.num_scan_obs == 0 else ScanEncoder(
            num_scan_obs = num_scan_obs,
    ...
    def update_distribution(self, obs_buf, privileged_obs_buf, estimated_obs_buf, scan_obs_buf, adaptation_mode=False):
        ...
        # Skip scan encoder if no scan observations
        if self.num_scan_obs == 0 or scan_obs_buf is None:
            actor_input = torch.cat((obs_buf, priv_latent, estimated_obs_buf), dim=-1)
        else:
